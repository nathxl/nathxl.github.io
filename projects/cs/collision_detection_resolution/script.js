...
const circle = new Circle(new Vector2D(435, 300), 30);
const circle_2 = new Circle(new Vector2D(162, 200), 50);
const rect = new Rectangle(new Vector2D(112, 300), 100, 50);
const rect_3 = new Rectangle(new Vector2D(385, 150), 100, 50);
...
function handle_input() {
    if (pressedKeys['q']) player.apply_force(new Vector2D(-1, 0), delta_time);
    if (pressedKeys['d']) player.apply_force(new Vector2D(+1, 0), delta_time);
    if (pressedKeys['z']) player.apply_force(new Vector2D(0, -1), delta_time);
    if (pressedKeys['s']) player.apply_force(new Vector2D(0, +1), delta_time);
}
...
function draw() {
    ...
    ctx.rect(50, 100, 225, 300); // Add a rectangle to the current path
    ctx.stroke();

    ctx.beginPath(); // Start a new path
    ctx.rect(325, 100, 225, 300); // Add a rectangle to the current path
    ctx.stroke();


    ctx.fillStyle = 'red';
    ctx.font = "25px Arial";
    ctx.fillText("detection", 50, 425);
    ctx.fillText("resolution", 325, 425);
