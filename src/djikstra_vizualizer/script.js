...
const A = new Vector(70, 70);
const B = new Vector(50, 300);
const C = new Vector(300, 125);
const D = new Vector(200, 325);
const E = new Vector(500, 150);
const F = new Vector(300, 300);
const G = new Vector(400, 400);
const H = new Vector(550, 450);
...
function get_node_clicked() {
    for (const node of graph.keys()) {
        if (node.distanceTo(mouse_pos) < POINT_SELECTION_DISTANCE) {
            return node;
        }
    }
    return null;
...
canvas.addEventListener("mousedown", (event) => {
    mouse_down = true;
    mouse_pos = new Vector(event.x - mouse_offset_x, event.y - mouse_offset_y);
    ...
    if (mode == 'add_point' && selected_node == null) {
        let new_point = new Vector(mouse_pos.x, mouse_pos.y);
...
canvas.addEventListener("mousemove", (event) => {
    let new_mouse_pos = new Vector(event.x - mouse_offset_x, event.y - mouse_offset_y);
    if (mouse_down) {
        if (mode != 'add_arc') {
