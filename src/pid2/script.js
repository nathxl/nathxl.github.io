...
const sp2 = new SubParticle(new Vector(150, 50, 0), 1);
const sp3 = new SubParticle(new Vector(150, 100, 0), 1);
const sp4 = new SubParticle(new Vector(50, 100, 0), 1);
const rb2 = new RigidBody2([sp1, sp2, sp3, sp4]);


let target = new Vector(canvas.width / 2, canvas.height / 2, 0);
...
function apply_gravity(rigid_body) {
    const force = (new Vector(0, 1, 0)).multiply(g);
    rigid_body.applyForce(ctx, new Vector(0, 0, 0), force);
}
...
function left_thrust(val) {
    const force = (new Vector(0, -1, 0)).rotateAroundZ(rb2.theta).unit().multiply(val);
...
function handleInput(rigidBody) {
    ...
    // boosters
    if (pressedKeys['w']) {
        const force = (new Vector(0, -1, 0)).rotateAroundZ(rigidBody.theta).unit().multiply(25);
